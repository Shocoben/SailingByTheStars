...
namespace Ogre
{
        ...
        int TerrainMaterialGeneratorB::SM2Profile::getNumberOfLightsSupported() const
    {
                ...
                const MaterialPtr& mat, const Terrain* terrain, TechniqueType tt)
        {
                ...
                //! @todo caustics still don't work
                //! @todo maybe add optin to set here
                if(true)
                {
                        TextureUnitState* tu = pass->createTextureUnitState();
                        tu->setAnimatedTextureName("Caustic.bmp",16,1.5);
                        tu->setColourOperationEx(LayerBlendOperationEx::LBX_MODULATE,LayerBlendSource::LBS_CURRENT,LayerBlendSource::LBS_TEXTURE);
                        tu->setTextureScale(0.1,0.1);
                }
        }
        //---------------------------------------------------------------------
                ...
                const SM2Profile* prof, const Terrain* terrain, TechniqueType tt, const GpuProgramParametersSharedPtr& params)
        {
                ...
                Vector4 scaleBiasSpecular(0.03, -0.04, 32, 0.5);
